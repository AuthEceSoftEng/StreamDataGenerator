#!/usr/bin/env python3
"""
Script to run all examples from the ./examples_nl_descriptions directory.
...
from generate_sdg import generate_sdg_from_file
from modelapi import create_provider_from_env
...
def main():
    ...
    examples_dir = script_dir / "examples_nl_descriptions"
    results = []

    print("=" * 80)
    print("Running Natural Language Text to SDG Examples")
    print("=" * 80)
    print()

    # Find all .txt files (excluding test files)
    txt_files = sorted([f for f in examples_dir.glob("*.txt") if not f.name.startswith("test_")])
    if not txt_files:
        print("❌ No .txt files found in examples directory")
        return 1

    # Create a single provider from environment and reuse it for all conversions.
    # This avoids creating a new connection per file and lets us surface auth errors
    ...
    for txt_file in txt_files:
        ...
        sdg_file = txt_file.with_suffix('.sdg')
        try:
            # Pass the reusable provider if available so errors are surfaced here
            if provider:
                generate_sdg_from_file(txt_file, provider=provider)
            else:
                generate_sdg_from_file(txt_file)
            print(f"  ✅ Creation of .sdg file: OK")
            creation_summary.append((txt_file.name, True, None))
        except Exception as e:
            # Don't abort the whole run on a single failure; record and continue.
            print(f"  ❌ Creation of .sdg file: FAILED - {e}")
            creation_summary.append((txt_file.name, False, str(e)))

        # TODO: Cleanup generated file later, not here!
